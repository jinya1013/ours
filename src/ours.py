...
class PrototypeLoss(nn.Module):
    ...
    def forward(self, feature, prototype_vectors, class_):
        distances = []
        for k, prototype in prototype_vectors.items():
            if prototype is not None:
                distance = torch.norm(
                    feature - prototype, dim=1
                )  # Calculate Euclidean distance
            else:
                distance = float("inf")  # Set distance to infinity if prototype is None
            distances.append(distance)

        distances = torch.stack(distances, dim=1)
        loss = distances[class_] + torch.log(torch.sum(torch.exp(-distances), dim=1))
...
class ResNet(nn.Module):
    ...
    def predict(self, x, prototype_vectors):
        ...
        for class_, prototype_vector in prototype_vectors.items():
            ...
            # print(self.task_id, self.conv1.weight[:5], self.tasks_masks[self.task_id][0][:5])
            # print(class_, self.task_id)
            feature = self.features(x)
            # print("class: ", class_, "task", self.class_task_map[class_], "\npv", prototype_vector[:5], "\nfv", feature[:, :5])
            distance = euclidean_distance(feature, prototype_vector).to(self.device)
            # print("DIST", distance)
            # print(distance.device, min_distance.device, y_preds.device)
            y_preds = torch.where(
...
# def accuracy(model, data_loader, prototype_vectors, device):
#     correct_preds = 0
#     n = 0
...
def validate(valid_loader, prototype_vectors, model, criterion, task_id, device):
    ...
    for X, y_true in valid_loader:
        ...
        # Forward pass
        y_hat, features = model(X, prototype_vector=X_prototype)

        loss = criterion(y_hat, y_true, features)
    ...
    for epoch in range(0, epochs):
        ...
        with torch.no_grad():
            model, valid_loss = validate(
                valid_loader, prototype_vectors, model, criterion, task_id, device
            )
        ...
        train_acc = accuracy(model, train_loader, prototype_vectors, device=device)
        valid_acc = accuracy(model, valid_loader, prototype_vectors, device=device)
