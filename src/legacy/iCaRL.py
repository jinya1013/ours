...
class Exemplar:
    def __init__(self, exemplar_size):
        self.exemplar_set = []
        self.size = exemplar_size

    def reduce_exemplar_set(self, index, m):
        self.exemplar_set[index] = self.exemplar_set[index][:m]
        print(
            "Size of class %d exemplar: %s"
            % (index, str(len(self.exemplar_set[index])))
        )

    def construct_exemplar_set(self, feature_map, images, m):
        features = feature_map(images)
        class_mean = features.mean()
        exemplar = []
...
class iCaRL:
    def __init__(self, model, exemplar_size, num_class):
        self.model = model
        self.exemplar = Exemplar(exemplar_size)
        self.num_class = num_class

    def classify(self, images):
        f = self.model.feature_map(images)
        mu = [
            self.model.feature_map(e).mean(axis=0) for e in self.exemplar.exemplar_set
        ]
        _, labels = torch.norm(f - mu[None]).min()
        return labels
    ...
    def step(self, x):
        ...
        x: tensor(K, C, H, W)
        """
        self.model.update_repr(x, self.exemplars)
        m = self.exemplar.size / (self.num_class + len(x))
        for y in range(0, self.num_class):
            self.exemplar.reduce_exemplar_set(y, m)
        for y in range(0, len(x)):
            self.exemplar.construct_exemplar_set(self.model.feature_map, x[y], m)
...
class iCaRLmodel:
    ...
    def _get_train_and_test_dataloader(self, classes):
        ...
        train_loader = DataLoader(
            dataset=self.train_dataset, shuffle=True, batch_size=self.batchsize
        )

        test_loader = DataLoader(
            dataset=self.test_dataset, shuffle=True, batch_size=self.batchsize
        )
        ...
        for epoch in range(self.epochs):
            if epoch == 48:
                if self.numclass == self.task_size:
                    print(1)
                    opt = optim.SGD(
                        self.model.parameters(), lr=1.0 / 5, weight_decay=0.00001
                    )
                else:
                ...
                if self.numclass > self.task_size:
                    ...
                    opt = optim.SGD(
                        self.model.parameters(), lr=1.0 / 25, weight_decay=0.00001
                    )
                print("change learning rate:%.3f" % (self.learning_rate / 25))
            elif epoch == 80:
                if self.numclass == self.task_size:
                    opt = optim.SGD(
                        self.model.parameters(), lr=1.0 / 125, weight_decay=0.00001
                    )
                else:
                    for p in opt.param_groups:
    ...
    def afterTrain(self, accuracy):
        ...
        for i in range(self.numclass - self.task_size, self.numclass):
            print("construct class %s exemplar:" % (i), end="")
